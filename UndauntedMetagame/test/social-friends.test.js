...
test("accepted Slayer Link reserves a slot for both friends, persists, and refuses outsiders", async () => {
    ...
    assert.equal((await request(john, "/slayerlink/invite", "PUT", { account_id: manda.UserId, slot: 1 })).status, 403);
    ...
    assert.equal((await request(john, "/slayerlink/invite", "PUT", { account_id: manda.UserId, slot: 0 })).status, 400);
    assert.equal((await request(john, "/slayerlink/invite", "PUT", { account_id: manda.UserId, slot: 4 })).status, 400);
    const response = await request(john, "/slayerlink/invite", "PUT", { account_id: manda.UserId, slot: 1 });
    assert.equal(response.status, 200);
    let id = (await response.json()).payload.link_id;
    ...
    assert.equal(incoming[0].account_id, john.UserId);
    assert.equal(incoming[0].direction, "Received");
    assert.equal(incoming[0].status, "Pending");
    assert.equal((await request(john, "/slayerlink/invite", "POST", { account_id: manda.UserId, action: "cancel" })).status, 200);
    assert.deepEqual((await (await request(manda, "/slayerlink/invites")).json()).payload.invites, []);
    id = (await (await request(john, "/slayerlink/invite", "PUT", { account_id: manda.UserId, slot: 1 })).json()).payload.link_id;
    ...
    assert.equal(a[0].account_id, manda.UserId);
    assert.equal(b[0].account_id, john.UserId);
    assert.equal(a[0].slot, 1);
    assert.equal(b[0].slot, 1);
    // Native fields plus the optional generation token for guarded writes.
    assert.deepEqual(Object.keys(a[0]).sort(), ["account_id", "ends", "link_id", "prize_pool", "slot"]);
    assert.equal(a[0].link_id, id);
    assert.equal(b[0].link_id, id);
    assert.equal(a[0].ends, b[0].ends);
    assert.equal(db.select().from(context.Schema.slayerlinks).all().length, 1);
    assert.equal((await request(john, "/slayerlink/invite", "PUT", { account_id: manda.UserId, slot: 1 })).status, 409);
    assert.equal((await request(todd, "/slayerlink/link", "DELETE", { slot: 1 })).status, 200);
